...
export default function Button({ className }: { className?: string }) {
  return (
    <button
      className={twMerge(
        'relative group flex flex-row item-center justify-center p-3 bg-[#2A30EE] hover:bg-[#1E4FD8] border border-[#2A30EE] transition-all duration-300 w-[200px] shadow-xl hover:shadow-none font-bold rounded-full cursor-pointer',
        className,
      )}
      onClick={() => {
    ...
    >
      ...
      <span className="group-hover:translate-x-1 transition-all duration-300 opacity-90">
        Trade now
      </span>
    </button>
  );
